(function(window, angular, undefined){
	 ...
	 */
	var moduleArr = ['directive','filter','util','url','api','baseConstant','baseCode','storage'];
	var admin = false;
	if(admin){
		//moduleArr.push('grid');
	}
	...
	app.controller('KaisaController',['$rootScope', 'commonParam' ,'$window','$scope','$location','$compile','$http','$timeout','kaisaUrl','kaisaParam','constant','kaisaApi','kaisaStorage','$httpParamSerializerJQLike','$filter',
		...
		$scope.jsonpParam = function(param){
			...
			return '?' + $httpParamSerializerJQLike(param);
		};	
		$scope.menuList = [
		...
		$scope.adminMenuList = [
			{title:'예약내역', url:'/admin/reservation'},
			{title:'문의내역', url:'/admin/qna'},
			{title:'공지사항', url:'/admin/notice'},
			{title:'자주하는질문', url:'/admin/faq'},
			{title:'휴일설정', url:'/admin/holiday'}
		];
		/**
		 * @param param : {
