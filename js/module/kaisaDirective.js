	...
	app.directive('kaisaAdminMenu',[function(){
		return {
			template: '<div id="adminMenu">'+
				'<ul>'+
					'<li data-ng-repeat="(idx, i) in adminMenuList"><a data-ng-href="/{{i.url}}">{{i.title}}</a></li>'+
				'</ul>'+
			'</div>',
			replace: true,
			link: function($scope, el, attrs){
		...
		return {
			template: '<div>'+
				...
				'<div id="loading" data-ng-if="loading.active" data-ng-class="{on:loading.active}">'+
					...
					'</ul>'+
				'</div>'+
				'<form id="login" data-ng-if="admin.layer.active" data-ng-submit="admin.submit()">'+
					'<h3>로그인</h3>'+
					'<input autocapitalize="off" autocomplete="off" type="text" maxlength="10" required="required" data-ng-model="admin.id" />'+
					'<input autocapitalize="off" autocomplete="off" type="password" maxlength="10" required="required" data-ng-model="admin.password" />'+
	...
	app.directive('kaisaFooter',[function(){
		return {
			template: '<div id="footer">'+
				...
				'copyright (c) 2018 chloris. all right reserved<span data-ng-click="admin.layer.open()">.</span></p>'+
